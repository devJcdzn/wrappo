export class Wrappo {
  ...
  private extractErrorDetails(args: any[]): any {
    try {
      const error =
      ...
      const relevantStackLines = stackLines.filter(
  ...
  private displayWrappoError(error: any) {
    console.log(
      "\x1b[31m🔴 Erro detectado!\x1b[0m",
      error.type,
      "em",
      error.location
    );
  ...
  private displayWrappoWarning(error: any) {
    console.log(
      "\x1b[33m🟡 Aviso detectado!\x1b[0m",
      error.type,
      "em",
      error.location
    );
  ...
  private getFixSuggestion(message: string): string {
  ...
  private async sendToServer(type: "error" | "warn", error: any) {
